 ...
 * @author Uwe Krueger
 */
public abstract class GalleryPanel<E,M extends ThumbnailListModel<E>> extends GBCPanel {
  ...
  public GalleryPanel(M model)
  {
    this(model,1,null);
  }
  
  public GalleryPanel(M model, int rows, Dimension d)
  {
    if (d!=null) maximage=d;
    System.out.println("Gallery panel for items of size "+Dimensions.toString(d));
  ...
  {
    System.out.println("gallery item size "+Dimensions.toString(item));
    super.panelBound();
    if (model!=null) {
  ...
  {
    System.out.println("cleanup gallery");
    super.panelUnbound();
    if (model!=null) {
      if (model instanceof ProxyModel) {
