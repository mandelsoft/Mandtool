...
import com.mandelsoft.mand.meth.OptimizedBigDecimalMandIteratorExp;
 ...
 * @author Uwe Krueger
 */
public class MandExp extends Command implements PixelIterator.PropertySource.PropertyHandler {
  ...
  public MandExp(MandelData md, QualifiedMandelName n) {
    this.md = md;
    this.name = n;
    this.mi = md.getInfo();
  ...
  public MandExp(MandelData md, QualifiedMandelName n, Environment env)
          throws IOException {
    this(md, n);
    this.env = env;
  ...
  public MandExp(File f, Environment env) throws IOException {
    this(new MandelData(f), QualifiedMandelName.create(f), env);
  }

  public MandExp(MandelData md, MandelData old, File out, QualifiedMandelName n,
                                           ...
                                           Map<String, String> properties)
  {
    ...
    System.out.println("experimental iteration mode");
    return new OptimizedBigDecimalMandIteratorExp(x0, y0, dx, dy, rx, ry, limit, properties);

  }
  
  private void setupContext() {
    pi = createPixelIterator(mi, this);
        ...
        for (File fi : files) {
          ...
          try {
            ...
            if (req.getHeader().hasImageData()) {
              ...
              m = new MandExp(req, old, file, name, env);
            }
  ...
  private static class Service {
    ...
    int handle(MandelHandle mh, boolean fallback) {
      ...
      try {
        ...
        /////////////// 
        MandExp m = new MandExp(req, old, file, name, env);
        m.setFilter(filter);
        if (this.mflag) {
