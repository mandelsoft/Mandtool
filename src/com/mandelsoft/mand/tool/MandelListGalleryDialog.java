 ...
 * @author Uwe Krueger
 */
public class MandelListGalleryDialog extends MandelDialog {
  ...
  private MandelListGalleryPanel gallery;

  public MandelListGalleryDialog(MandelWindowAccess owner,
                            ...
                            AbstractMandelListListModel lmodel, String name)
  {
    ...
    if (mp!=null && mp.getParentColormapModel().isSet()) {
      model.setColormapSourceFactory(
        new CachedUpstreamColormapSourceFactory(owner.getEnvironment().getAreaColormapScanner(),
                                                mp.getColormapModel(),
                                owner.getEnvironment().getColormapCache()));
      System.out.println("-> gallery with upstream colormap");
    }
    else {
      System.out.println("-> gallery with main colormap");
    }

    gallery=new MandelListGalleryPanel(model);
    if (owner.getMandelImagePane()!=null) {
      gallery.setMaxFrame(owner.getMandelImagePane().getMaxFrame());
    }
    add(gallery);
    pack();
  ...
  {
    gallery.setRootName(rootName);
  }

  public MandelName getRootName()
  {
    return gallery.getRootName();
  }
