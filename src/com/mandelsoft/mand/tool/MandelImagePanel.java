...
import com.mandelsoft.mand.tool.Merger.MergeBase;
...
import com.mandelsoft.swing.ListModelMenu;
import com.mandelsoft.swing.MovableScrollPane;
...
import com.mandelsoft.swing.WorkerProgressDialog;
...
public class MandelImagePanel extends GBCPanel
    ...
    public AreaSelectionModel() {
      ...
      this.addListDataListener(new AbstractListDataListener() {
        ...
        public void contentsChanged(ListDataEvent e) {
          if (debug) {
            System.out.println("select SELECTOR " + getSelectedItem());
          }
          if (!isReadonly()) {
  ...
  public class ProportionSelectionModel extends DefaultComboBoxModel {
    ...
    public ProportionSelectionModel() {
      ...
      this.addListDataListener(new AbstractListDataListener() {
        ...
        public void contentsChanged(ListDataEvent e) {
          if (debug) {
            System.out.println("select PROPORTION " + getSelectedItem());
          }
          if (!isReadonly()) {
  ...
  /////////////////
  // Area Coordinates
  public class CoordinatesToolTipHandler implements
    ...
    public String getToolTipText(MouseEvent event) {
      MandelRaster r;
      ColorModel m;
      BigDecimalMandIterator it = new BigDecimalMandIterator(getMandelInfo());
      BigDecimal cx = it.getCX((int) (event.getX() / filterscale));
      BigDecimal cy = it.getCY((int) (event.getY() / filterscale));
      return String.format("(%s,%s)", cx, cy);
    }
  }
      ...
      if (sub != null) {
        ...
        if (mn != null && !mn.equals(last)) {
          ...
          if (getEnvironment().isReadonly()) {
            if (ic == 1) {
              text = sub + " (" + ic + " sub area)";
            } else {
              text = sub + " (" + ic + " sub areas)";
            }
          } else {
            int ir = count(mn, getEnvironment().getInfoScanner());
            if (ic == 1) {
              text = sub + " (" + ic + " sub area, " + ir + " pending)";
            } else {
              text = sub + " (" + ic + " sub areas, " + ir + " pending)";
            }
          }
        }
      } else {
  ...
  public class ToolTipSelectionModel extends DefaultComboBoxModel {
    ...
    public ToolTipSelectionModel() {
      ...
      this.addListDataListener(new AbstractListDataListener() {
        ...
        public void contentsChanged(ListDataEvent e) {
          if (debug) {
            System.out.println("select ToolTip " + getSelectedItem());
          }
          wrapper.setToolTipHandler(getActualToolTipHandler());
        }
      });

      addElement(def = new ToolTipFlavor(Settings.PIXEL_TOOLTIP_PIXELINFO, "Pixel Info",
              new PixelInfoToolTipHandler()));
      addElement(new ToolTipFlavor(Settings.PIXEL_TOOLTIP_SUBAREANAME, "SubArea Name",
              new SubAreaToolTipHandler()));
      addElement(new ToolTipFlavor(Settings.PIXEL_TOOLTIP_COORDINATES, "Area Coordinates",
              new CoordinatesToolTipHandler()));
...
//      wrapper.setToolTipHandler(def.getToolTipHandler());
  ...
  public static class RefCoord {
    ...
    public RefCoord(QualifiedMandelName n, Coord c, int d) {
      coord = c;
      area = n;
      depth = d;
    }
  ...
  public MandelVariantModel getMandelVariantModel() {
    return vmodel;
  }
  ...
  private class VariantListener implements ChangeListener {
    ...
    public void stateChanged(javax.swing.event.ChangeEvent e) {
      if (!adjusting) {
        ...
        adjusting = true;
        setImage(vmodel.getVariantHandle());
        adjusting = false;
      } else {
        if (debug) {
          System.out.println("ignore remap update");
        }
      }
    }
  }

  private class VariantMenu extends ListModelMenu {

    private MandelVariantModel model;

    public VariantMenu(String name, MandelVariantModel model) {
      super(name, model);
      setMinSize(2);
      this.model = model;
    ...
    // Attention: Hack mapping the combo box information to menu items
    //            The variant model does not offer the handles as model elements
    //            but strings
    @Override
    protected String getText(Object e) {
      String q = ((MandelHandle) e).getQualifier();
      if (q != null) {
        return q;
      }
      return "<standard>";
    ...
    protected Object getElementAt(int index) {
      return model.getHandleAt(index);
    }

    @Override
    protected String getActionCommand(Object elem) {
      return model.getLabel((MandelHandle) elem);
    }

    @Override
    protected int compare(Object e1, Object e2) {
      if (((MandelHandle) e1).getQualifier() == null) {
        return -1;
      }
      if (((MandelHandle) e2).getQualifier() == null) {
        return 1;
      }
      return ((MandelHandle) e1).getQualifier().compareTo(((MandelHandle) e2).getQualifier());
    }

    @Override
    protected boolean isEnabled(Object elem) {
      return model.getVariantHandle() != elem;
    }
    ...
    private class Listener implements ActionListener, ChangeListener {
      ...
      public void actionPerformed(ActionEvent e) {
        model.setSelectedItem(e.getActionCommand());
      }
  ...
  private Action rememberDeepestRefCoordAction;
  ...
  private Action linksGalleryAction;
  private Action keyareasGalleryAction;
  private Action zoomGalleryAction;
  private Action cloneAction;
  private Action gotoAction;
  private Action saveAction;
  private Action rasterSaveAction;
  private Action areaCMSaveAction;
  private Action areaCMDeleteAction;
  private Action areaCMAction;
  private Action cmAction;
  ...
  public MandelImagePanel(ToolEnvironment env,
    ...
    this.scrollpane = new MovableScrollPane(buffer);
    ...
    //JLayer<JScrollPane> layers = new JLayer(scrollpane, new ScrollLayer<JScrollPane>());
    add(scrollpane, GBC(0, 0, GBC.BOTH));
  ...
  public int getValueAtMouse() {
    MandelData d = getMandelData();
    if (d == null) {
      return 0;
    }
    return d.getRaster().getData(mouseX, mouseY);
  }

  
  public Coord getCoordinatesAtMouse() {
    BigDecimalMandIterator i = new BigDecimalMandIterator(getMandelInfo());
    return new Coord(i.getCX(mouseX), i.getCY(mouseY));
  }

  public Coord getCoordinatesAt(int cx, int cy) {
    BigDecimalMandIterator i = new BigDecimalMandIterator(getMandelInfo());
    return new Coord(i.getCX(cx), i.getCY(cy));
  }
  ...
  protected void panelBound() {
    ...
    buffer.addScaleEventListener(new ScaleAdapter() {
      ...
      public void componentScaled(ScaleEvent e) {
        setTitle();
        if (debug) {
          System.out.println("buf: " + getImagePane().getPreferredSize());
        }
        if (debug) {
          System.out.println("VP: " + getScrollPane().getViewport().getExtentSize());
        }
    ...
    rememberDeepestRefCoordAction = new DeepestRefCoordAction();
    ...
    linksGalleryAction = new LinksGalleryAction();
    keyareasGalleryAction = new KeyAreasGalleryAction();
    zoomGalleryAction = new ZoomGalleryAction();
    cloneAction = new CloneAction();
    gotoAction = new GotoAction();
    if (!isReadonly()) {
      saveAction = new SaveAction();
      rasterSaveAction = new RasterSaveAction();
      areaCMSaveAction = new AreaCMSaveAction();
      areaCMDeleteAction = new AreaCMDeleteAction();
    }
    areaCMAction = new AreaCMAction();
    cmAction = new CMAction();
...
//    JMenu memory=null;
    ...
    if (!env.isReadonly()) {
      JMenu adjust = new UpdatableJMenu("Adjust");
      it = new UpdatableJMenuItem(cleanAction);
      adjust.add(it);
      it = new UpdatableJMenuItem(mergeAction);
      adjust.add(it);
      menu.add(adjust);
    }
    ...
    menu.add(new VariantMenu("Variants", getMandelVariantModel()));

    JMenu gallerymenu = new JMenu("Galleries");
    gallerymenu.add(linksGalleryAction);
    gallerymenu.add(keyareasGalleryAction);
    gallerymenu.add(zoomGalleryAction);
    ...
    it = new JMenuItem(linksGalleryAction);
    it.setText("Gallery");
    linkmenu.add(it);
    menu.add(gallerymenu);
    menu.add(linkmenu);
    ...
    menu.add(it);
    it = new UpdatableJMenuItem(listsDialog.BackAction());
    menu.add(it);
    it = new UpdatableJMenuItem(listsDialog.ForthAction());
    menu.add(it);
    ...
    refmenu.add(it);
    it = new JMenuItem(rememberDeepestRefCoordAction);
    refmenu.add(it);
    ...
    if (iterPathActive) {
      menu.add(showIterationPathAction);
    }
    ...
    submenu.add(new UpdatableJMenuItem(cmAction));
    ...
    if (getWindow() instanceof JFrame) {
      it = new UpdatableJMenuItem(new CloseAction());
      menu.add(it);
    }
  ...
  private MandelListTableModel mergeModel;
  ...
  public MandelListFolderTreeModel getMandelBrowserModel() {
    if (browserModel == null) {
      ...
      local.addListModel(getMergeModel(), "merge");
  ...
  public MandelListTableModel getMergeModel() {
    if (mergeModel == null) {
      mergeModel = new DefaultMandelListTableModel(new DefaultMandelList(), getEnvironment().getImageDataScanner());
      mergeModel.setModifiable(true);
      mergeModel.addMandelListListener((ChangeEvent evt) -> {
        mergeAction.updateObject(null);
      });
    }
    return mergeModel;
  }
  ...
  private void updateKeyAreas() {
    ...
    keyareasGalleryAction.setEnabled(!keyareas.getList().isEmpty());
  }
  ...
  private void updateLinks() {
    ...
    linksGalleryAction.setEnabled(m != null && !m.getList().isEmpty());
  ...
  private void setImageData(MandelAreaImage img, boolean showlast) {
    ...
    if (highlight_mode == H_LAST) {
      highlighted.setName(lastname);
    }
    this.name = name;
    this.image = img;
  ...
  synchronized public MandelColormapCache getColormapCache() {
  ...
  private class PanelColormapSource extends CachedUpstreamColormapSource {
    ...
    //@Override
    protected Colormap XoptimizedLoad(MandelHandle h) {
  ...
  private boolean setImage(ImageFactory fac, boolean showlast) {
    try {
      ...
      if (img != null) {
        ...
        mandelInfo("No image for " + fac + " found.");
      }
    } catch (IOException ex) {
...
//    }
  ...
  private boolean showSubRect(MandelHandle h, boolean vis) {
    ...
    if (data == null) {
      ...
      if (rect == null) {
        rect = getImagePane().createRect(n.getName(),
                getLabel(n), info);
      }
      updateRect(rect, info);
  ...
  private boolean highlight(MandelName n) {
    if (highlighted.isActive() && highlighted.getMandelName().equals(n)) {
      return true;
    }
  ...
  public class ShowRefCoordAction extends RefCoordActionBase {
    ...
    protected int getDepth() {
      ...
      return remembered.refcoord.depth;
    }
  ...
  private class MouseRefCoordAction extends UpdatableAction {
    ...
    public void actionPerformed(ActionEvent e) {
      ...
      int v = MandelImagePanel.this.getValueAtMouse();
      if (v > 0) {
        remembered.refcoord = new RefCoord(getQualifiedMandelName(), c, v);
      } else {
  ...
  private class DeepestRefCoordAction extends UpdatableAction {

    public DeepestRefCoordAction() {
      super("Remember Deepest");
    }

    public void actionPerformed(ActionEvent e) {
      MandelData d = MandelImagePanel.this.getMandelData();
      if (d != null && d.getRaster() != null) {
        IntMatrix m = d.getRaster().getRaster();

        int max = 0;
        int mx = 0;
        int my = 0;
        for (int y = 0; y < m.getRY(); y++) {
          for (int x = 0; x < m.getRX(); x++) {
            int v = m.getData(x, y);
            if (v > max) {
              max = v;
              mx = x;
        ...
        System.out.printf("max iteration value at (%d,%d) = %d\n", mx, my, max);
        Coord c = MandelImagePanel.this.getCoordinatesAt(mx, my);
        remembered.refcoord = new RefCoord(getQualifiedMandelName(), c, max);
        updateObjects("refcoord", remembered.refcoord);

        if (!showRefCoordAction.isActive()) {
          showRefCoordAction.actionPerformed(null);
        }
      }
    }
  }
  ...
  ////////////////////////////////////////////////////////////////////////////
   ...
   private class CleanAction extends UpdatableAction {
    ...
    public void actionPerformed(ActionEvent e) {
      ...
      setBusy(true);
      int found = c.clean(imageDialog.getCleanLimit(), imageDialog.getDetectionNeighborhood());
      System.out.printf("*** found %d fixed black pixels", found);
      if (found != 0) {
        ((MandelImage.Updatable) image.getMandelImage()).updateRaster();
        updateObjects("raster", null);
      }
      setBusy(false);
  ...
  ////////////////////////////////////////////////////////////////////////////
   ...
   private class MergeWorker extends SwingWorker<Integer, Void> implements Merger.Progress {
    MandelListFolder folder;
    int max;
    int current;


    MergeWorker(MandelListFolder folder) {
      current = 0;
      this.max = folder.getMandelList().size();
      this.folder=folder;
    }

    @Override
    public void reportProgress(double p) {
      int v = (int) (((p + (double)current) * 100.0) / max);
      setProgress(v);
    }
    ...
    protected Integer doInBackground() throws Exception {
      int cnt = 0;
      current=-1;
      MergeBase base = new MergeBase(getQualifiedMandelName(), getMandelData());
      base.setProgressListener(this);
      base.setCancelCheck(this::isCancelled);
      for (QualifiedMandelName n : folder.allentries()) {
        MandelHandle h = getEnvironment().getImageDataScanner().getMandelHandle(n);
        current++;
        firePropertyChange("note", null, h.getName().toString());

        try {
          //progress.nextRaster(n);
          Merger j = base.createMerger(h);
          try {
            String err = base.merge(j);
            if (err != null) {
          ...
          System.out.printf("invalid area: %s\n", ex);
        }
      }
      if (cnt>0) base.finish();
      return cnt;
    }
  }
   
  private class MergeAction extends UpdatableAction {
    ...
    public void actionPerformed(ActionEvent e) {
      ...
      setBusy(true);
      System.out.printf("start merging\n");

      MandelListFolder folder = MandelListFolderTreeModel.Util.getFolder(getMandelBrowserModel(), "merge");
      if (folder != null) {
        System.out.printf("found general merge area list\n");
        
        MergeWorker w = new MergeWorker(folder);
        try {
        cnt = WorkerProgressDialog.show(getMandelImagePane(), w, "Merging Rasters", "preparing...", true);
        }
        catch (Exception ex) {
          mandelError("Failed: "+ex.getMessage());
        }
      }
      ...
      if (cnt > 0) {
        ((MandelImage.Updatable) image.getMandelImage()).updateRaster();
        updateObjects("raster", null);
        mandelInfo(String.format("merged %d rasters", cnt));
      }
      setBusy(false);
    }
    ...
    @Override
    public void updateObject(UpdateContext ctx) {
      MandelRaster r = getMandelData().getRaster();

      MandelListFolder folder = MandelListFolderTreeModel.Util.getFolder(getMandelBrowserModel(), "merge");
      if (folder != null) {
        for (QualifiedMandelName n : folder.allentries()) {
          if (n.equals(getQualifiedMandelName())) {
            continue;
          }
          try {
            MandelHandle i = env.getAutoImageDataScanner().getMandelInfo(n);
            if (i != null && i.getInfo().getInfo().hasOverlapWith(getMandelInfo())) {
              setEnabled(true);
              return;
            }
          } catch (IOException ex) {
  ...
  private class CommonAncestorAction extends MarkActionBase {
    ...
    protected void markActionPerformed(ActionEvent e) {
      ...
      while (!n.isAbove(marked.getMandelName())) {
        n = n.getParentName();
      }
      setImage(n, true);
    }
  ...
  private abstract class AboveMarkAction extends MarkActionBase {
    ...
    /**
     * prototype condition evaluation for all instances of AboveMarkAction not
     * overriding isPossible methods. Must be derived class to provide prototype
     * identity.
     */
    ...
    /**
     * must be overridden, also, if a derived class redefines isPossible(). keep
     * expensive condition evaluation in update context to reuse the evaluation
     * result for other derived classes sharing the same condition.
     *
  ...
  private class LinksGalleryAction extends OptionalAction {

    public LinksGalleryAction() {
      super("Links");
    }
    ...
    public void actionPerformed(ActionEvent e) {
      if (isPossible()) {
        ...
        String title = "Links Gallery for " + getMandelName()
                + " (" + Utils.sizeString(ml.size(), "entry") + ")";
        System.out.println(title);
        new MandelListGalleryDialog(MandelImagePanel.this.getFrameAccess(), ml, title);
      }
    }
  }

  private class KeyAreasGalleryAction extends AbstractAction {

    public KeyAreasGalleryAction() {
      super("Key areas");
    }
    ...
    public void actionPerformed(ActionEvent e) {
      ...
      if (m != null && !m.getList().isEmpty()) {
        MandelList ml = new DefaultMandelList(m.getList());
        String title = "Key Areas Gallery for " + getMandelName()
                + " (" + Utils.sizeString(ml.size(), "entry") + ")";
        System.out.println(title);
        new MandelListGalleryDialog(MandelImagePanel.this.getFrameAccess(), ml, title);
      }
  ...
  private class ZoomGalleryAction extends AbstractAction {

    public ZoomGalleryAction() {
      super("Zoom");
    }
    ...
    public void actionPerformed(ActionEvent e) {
      ...
      String title = "Zoom Gallery for " + getMandelName()
              + " (" + Utils.sizeString(ml.size(), "entry") + ")";
      System.out.println(title);
      new MandelListGalleryDialog(MandelImagePanel.this.getFrameAccess(), ml,
              title);
    }
  }
...
//      }
  ...
  private class RasterSaveAction extends UpdatableAction {

    public RasterSaveAction() {
      super("Save Raster");
    }

    public void actionPerformed(ActionEvent e) {
      if (getMandelData().getRaster().isModified()) {
        //MandelHandle h = getEnvironment().getImageDataScanner().getMandelHandle(getQualifiedMandelName());
        MandelHeader h = getMandelData().getOrigHeader();
        AbstractFile f = getMandelData().getFile();

        if (h.isRaster() && f.isFile()) {
          try {
            System.out.printf("updating raster file %s\n", f.getFile());
            MandelData md = new MandelData(getMandelData().getInfo());
            md.setRaster(getMandelData().getRaster());
            md.write(f.getFile());
            MandelFolder mf = MandelFolder.getMandelFolder(f.getFile());
            mf.update(f.getFile(), md);

            getMandelData().getRaster().setModified(false);
          } catch (IOException ex) {
            mandelError("raster cannot be saved", ex);
          }
        }
      }
    }
    ...
    public void updateObject(UpdateContext ctx) {
      MandelData d = getMandelData();
      setEnabled(d.getOrigHeader().isRaster() && d.getRaster().isModified());
    }
  }
  ...
  private class CMAction extends UpdatableAction {
    ...
    public void updateObject(UpdateContext ctx) {
      ...
      MandelName n = getMandelName();
      Set<MandelHandle> set = env.getImageDataScanner().getMandelHandles(getQualifiedMandelName());
      if (set != null) {
        for (MandelHandle mh : set) {
          if (mh.getHeader().hasColormap()) {
            b = true;
          }
        }
      }
      ...
      if (!b) {
        do {
          set = env.getAreaColormapScanner().
                  getMandelHandles(n);
          if (set != null) {
            for (MandelHandle mh : set) {
              if (mh.getHeader().isAreaColormap()) {
              ...
              if (mh.getQualifier() == null && mh.getHeader().hasColormap()) {
                b = true;
              }
            }
          }
      ...
      if (b) {
        name = n;
        putValue(Action.NAME, "eff: " + n.getName());
      } else {
        putValue(Action.NAME, "upstream");
      }
    }
  ...
  private class CloseAction extends UpdatableAction {
    ...
    public void updateObject(UpdateContext ctx) {
      ToolEnvironment env = getEnvironment();
      String mode = env.getProperty(Settings.PRESENTATION_MODE);
      if (mode != null && mode.equals("on")) {
        setEnabled(getEnvironment().getMandelWindowsModel().getSize()>1);
      }
    }
  }
  ...
  ////////////////////////////////////////////////////////////////////////////
   ...
   private boolean mandelQuestion(String msg) {
    return 0 == (int) JOptionPane.showConfirmDialog(MandelImagePanel.this,
            msg,
            "Question",
            JOptionPane.YES_NO_OPTION);
  }

  private boolean mandelQuestion(String title, String msg) {
    return 0 == (int) JOptionPane.showConfirmDialog(MandelImagePanel.this,
            msg,
            title,
            JOptionPane.YES_NO_OPTION);
  ...
  public class SelectAreaListener extends DefaultRectEventListener
    ...
    synchronized public void rectModified(RectModifiedEvent e) {
      VisibleRect rect = e.getRect();
      MandelInfo info;
      boolean created;
      ...
      if (e.getAction() == RectModifiedEvent.RECT_CREATED) {
        if (debug) {
          System.out.println("rect created " + rect
  ...
  synchronized public void updateRect(VisibleRect rect, MandelInfo info) {
    if (inupdate || getMandelInfo() == null) {
      return;
    }
...
//          rect.discard();
  ...
  public class InfoPopup extends RenderedComponent {
    ...
    public void setData(MandelInfo i, QualifiedMandelName name) {
      ...
      composed = i.hasProperty(MandelInfo.ATTR_COMPOSED_TIME);
      setMagnification(m);
      setInfo(name);
      setMin(i.getMinIt());
      setMax(i.getMaxIt());
    ...
    public void showAt(Component c, Point p) {
      ...
      if (debug) {
        System.out.println("show " + getBounds());
      }
